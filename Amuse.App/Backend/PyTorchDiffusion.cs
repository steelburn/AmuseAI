...
{
    public sealed class PyTorchDiffusion : ServiceBase, IDiffusionRuntime
    {
        ...
        public PyTorchDiffusion(Settings settings, IEnvironmentService environmentService, IMediaService mediaService, ILogger logger)
        {
            _logger = logger;
            _settings = settings;
        ...
        public async Task LoadAsync(PipelineModel pipeline, IProgress<PipelineProgress> progressCallback)
        {
            try
            {
                using (_cancellationTokenSource = new CancellationTokenSource())
                {
                    ...
                    if (!await LoadEnvironment(pipeline.Device, _pipelineConfig.DiffusionModel))
        ...
        {
            var environment = _environmentService.GetEnvironment(_pipelineConfig);
            var createOptions = environment.ToClientOptions(_settings, EnvironmentMode.Create);
            ...
            var pipelineConfig = _pipelineConfig.ToClientOptions(_settings);

            var relayedProgressCallback = new Progress<PipelineProgress>(progress => _progressCallback?.Report(progress));
            var diffusionPipeline = new PipelineClient(clientConfig, relayedProgressCallback, _logger);

            try
            {
                await diffusionPipeline.CreateAsync(createOptions, cancellationToken);
        ...
        {
            var environment = _environmentService.GetEnvironment(device, diffusionModel);
